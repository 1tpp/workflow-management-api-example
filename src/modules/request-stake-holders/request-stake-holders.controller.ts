...
export class RequestStakeHoldersController {
  constructor(
    private readonly requestStakeHoldersService: RequestStakeHoldersService,
  ) {}
  ...
  update(
    @Param('id') id: string,
    @Body() updateRequestStakeHolderDto: UpdateRequestStakeHolderDto,
  ) {
    return this.requestStakeHoldersService.update(
      +id,
      updateRequestStakeHolderDto,
    );
  }
